...
- ✅ **Fase 7: Advanced Security & WebRTC Audio Calling (SELESAI)** —
  ...
  2. **E2EE Single Active Device & Key Conflict Guard**: Pelacakan `active_device_id` & `key_version`, HTTP 409 Conflict rejection, explicit `/api/users/public-key/reset`, dan modal konflik UI (`DeviceConflictModal.tsx`) untuk memastikan konsistensi fingerprint Safety Number antar perangkat.
  3. **1-on-1 Voice / Audio Calling via WebRTC P2P (Milestone 7.2A)**: WebSocket signaling (`call_offer`, `call_answer`, `ice_candidate`), STUN + OpenRelay TURN fallback, Web Audio API procedural ringtones, In-Call Overlay UI dengan live timer dan microphone mute, serta modal panggilan masuk interaktif.
  4. **Security & Scalability Hardening**: Mitigasi BOLA/IDOR WebSocket (`isAuthorizedForRoom`), IDOR Media ACK check, Anti-SSRF Socket-Level IP Pinning (14 subnet), eliminasi $N+1$ query via $O(1)$ batched CTE window function, database indexing, dan SQLite WAL mode concurrency.
  5. *(Milestone 7.2B: 1-on-1 Video Calling di-hold sementara untuk memprioritaskan fitur inti komunikasi).*
