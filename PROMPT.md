...
- ⏳ **Fase 8: Core Parity (Push Notifications, Group Chat & Message Management) (SEDANG BERJALAN)** —
  ...
  12. ✅ **Milestone 8.2A: Group Chat Engine — Core Foundation (SELESAI)**: Percakapan multi-user (`conversations` & `conversation_members` tables), 9 REST endpoints grup, multicast WebSocket broadcast, role RBAC Admin/Member, modal buat grup (`CreateGroupModal.tsx`), `GroupInfoDrawer.tsx`, deterministic sender color di `MessageBubble.tsx`, identitas unik `grp_<UUIDv4>`, toggle publik/privat + `group_username` untuk public discovery, dan `parent_id` reserved untuk sub-grup.
  13. 🎯 **Milestone 8.2B: Member Management & Public Group Discovery (NEXT)**: Add member via username search + invite link, public group search by `group_username`/name, invite link generation & validation endpoint, UI modal Tambah Anggota, dan sub-group `parent_id` sebagai rencana jangka panjang.
  14. ⏳ **Milestone 8.2C: Bad Words Sensor Filter**: Sistem sensor kata-kata terlarang di pesan grup dan DM.
  15. ⏳ **Milestone 8.3: Message Management Suite**: Edit pesan (15 menit), forward pesan, pin chat & pin message, starred message, dan in-chat search.
