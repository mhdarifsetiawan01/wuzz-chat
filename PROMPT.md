...
- ✅ **Fase 8: Core Parity (Push Notifications, Group Chat & Message Management) (SELESAI)** —
  ...
  24. ✅ **Milestone 8.13: Direct WebSocket Kick on E2EE Key Transfer & Modal Hierarchy Hardening (SELESAI)**: Menendang koneksi WebSocket perangkat lama seketika saat `POST /api/users/transfer/consume` berhasil (`KickClientByUserID` dengan 500ms grace period & Close Code 4001 `SESSION_REPLACED`), mengikat CustomEvent `wuzz:session_replaced` pada `DeviceTransferModal` dan `ProfileModal`, menampilkan kartu transisi sukses & auto-dismiss 1.2 detik, serta menaikkan z-index `DeviceConflictModal` ke `1100` (`var(--z-modal-top)`). Dilindungi unit test Go & frontend build (100% PASS).
