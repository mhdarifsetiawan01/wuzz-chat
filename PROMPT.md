...
- ✅ **Fase 7: Advanced Security & WebRTC Audio Calling (SELESAI)** —
  ...
  3. **Zero-Knowledge QR Code Key Migration (Milestone 7.6 / Opsi 2)**: Fitur pemindahan identitas kriptografi antar perangkat via QR Code ephemeral (TTL 5 menit), enkripsi bundle AES-256-GCM berdasar 256-bit PBKDF2, atomic single-use retrieval di database SQL (`/api/users/transfer/create` & `/api/users/transfer/consume`), modal UI `DeviceTransferModal.tsx` dengan fallback kode manual, dan deep link `/transfer?token=...`.
