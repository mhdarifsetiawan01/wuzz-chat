...
- ⏳ **Fase 8: Core Parity (Push Notifications, Group Chat & Message Management) (SEDANG BERJALAN)** —
  ...
  13. ✅ **Milestone 8.2B: Ephemeral Sub-Groups & TTL Lifecycle Engine (SELESAI)**: Mini ruang diskusi bertopik di dalam grup induk dengan identitas unik `sub_<UUIDv4>` dan TTL otomatis (default 1 minggu `"7_days"`, opsi 1 bulan `"30_days"`). Dilengkapi **Strict Parent-Membership Gate** (fail-closed HTTP 403 / WS rejection bagi non-member grup induk), background daemon `SubGroupTTLWorker` (15m ticker) dengan batch expiration atomic, fail-closed read-only lock saat kedaluwarsa (WS block & UI input disabled), kesiapan AI Summary masa depan (`status` & `ai_summary`), penegakan identitas immutable murni UUID (DEC-008), dan antarmuka Aurora Glassmorphic (`SubGroupListDrawer.tsx`, `CreateSubGroupModal.tsx`, tombol status bar & drawer grup).
