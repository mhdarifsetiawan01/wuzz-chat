...
## DEC-008: Immutable-First Comparison & Identity Enforcement (UUID & Group ID Only)
- **Status**: Accepted
- **Context**: Pengguna dapat mengubah nama tampilan (`display_name`), username kontak, atau judul grup sewaktu-waktu. Jika logika perbandingan atau otorisasi memakai nilai-nilai mutable tersebut, sistem rentan terhadap spoofing, desinkronisasi hak akses, dan bug keamanan BOLA/IDOR.
- **Decision**:
  1. **Strict Immutable Identifier Enforcement**: Seluruh perbandingan hak akses, keanggotaan grup/subgrup, relasi induk, pemfilteran pesan, dan lookup di backend maupun frontend **WAJIB** menggunakan variabel immutable:
     - `users.id` (UUID format v4)
     - `conversations.id` (`grp_<UUIDv4>` atau `sub_<UUIDv4>`)
     - `conversations.parent_id` (`grp_<UUIDv4>`)
     - `conversation_members.user_id` (UUID format v4)
  2. **Prohibited for Comparison**: DILARANG KERAS menggunakan `username`, `display_name`, `nickname`, `title`, atau identifier lain yang mutable untuk memeriksa kepemilikan atau hak akses.
- **Consequences**:
  - Menjamin integritas data 100% konsisten dan anti-tampering.
  - Sesuai dengan standar UUID-First Identity Architecture yang telah diuji pada Milestone 8.7.
