...
- [x] In-App Live Camera QR Scanner: Implementasi pemindai kamera langsung via `expo-camera` (`CameraView`) dengan reticle viewfinder neon aurora, animasi laser scanning, torch toggle, dan auto-normalisasi multi-format untuk verifikasi instan Safety Number 30-digit.
...
- [x] **E2EE Key Conflict Handling**: Saat `PUT /api/users/public-key` mengembalikan HTTP 409 (`KEY_ALREADY_REGISTERED`), tampilkan dialog konfirmasi reset kunci (`KeyConflictModal`), verifikasi kata sandi, panggil `POST /api/users/public-key/reset` untuk generate keypair baru, atau batalkan secara lokal tanpa mengganggu sesi perangkat utama.
- [x] **WebRTC 1-on-1 Voice Call**: Signaling via WebSocket (`call_offer`, `call_answer`, `ice_candidate`, `call_reject`, `call_end`, `call_busy`), koneksi P2P via STUN/TURN, UI panggilan masuk `IncomingCallModal` & aktif `ActiveCallOverlay` bertema Aurora Dark Mode, manajemen rute audio Speakerphone vs Earpiece (`expo-audio`), runtime microphone permission guard (DEC-M31), dan lifecycle cleanup resource audio.
