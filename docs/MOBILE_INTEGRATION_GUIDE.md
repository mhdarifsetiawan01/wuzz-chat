...
- [x] **QR Code E2EE Device Transfer**: Implementasi `POST /api/users/transfer/create` (perangkat sumber) dan `POST /api/users/transfer/consume` (perangkat target) menggunakan kamera live native `CameraQRScannerModal` dan `QRCodeView` dengan PBKDF2 (100k iterasi, SHA-256) + AES-256-GCM zero-knowledge key wrapping — 100% interoperabel dengan klien Web.
