...
- [x] **Backend Logout Endpoint & Consolidated Encrypted Messages Banner (Fase 8 Milestone 8.10)**:
  - 🚪 **Pelepasan Sesi Perangkat Aktif & Proteksi Device-Aware (`POST /api/auth/logout`)**: Penambahan endpoint logout resmi di backend Go yang mengosongkan kolom `users.active_device_id = ''` di database secara kondisional (*device-aware*) saat logout sukarela (mengeliminasi false conflict HTTP 409 saat login di device baru). Dilengkapi proteksi pembatalan konflik (*local-only cancellation*): jika perangkat penantang/baru memilih *"Batalkan & Keluar"* di modal konflik, frontend hanya mengeksekusi pembersihan lokal tanpa memanggil server sehingga sesi perangkat aktif utama tetap aman terlindungi.
