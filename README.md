...
- [x] **Ephemeral Sub-Groups, TTL Lifecycle & Access Control Engine (Fase 8 Milestone 8.2B)**: Mini ruang obrolan bertopik di dalam grup induk dengan identitas unik `sub_<UUIDv4>` dan TTL otomatis. Dilengkapi dengan:
  - **Strict Parent-Membership Gate**: Proteksi fail-closed yang memastikan hanya anggota aktif grup utama yang dapat membuat, melihat daftar, bergabung, dan berpartisipasi di subgrup.
  - **Sub-Group Access Control (Terbuka vs Privat)**: Dukungan subgrup 🌐 **Terbuka** (anggota induk dapat langsung gabung) vs 🔒 **Privat** (memerlukan izin admin atau undangan). Semua subgrup tetap dapat dilihat di daftar (*transparent discovery*).
  ...
  - **Auto-Purge Data Kedaluwarsa**: Ketika subgrup mencapai masa expired, data di `conversation_join_requests` otomatis dihapus tuntas oleh `ExpireSubGroupsBatch`.
  ...
  - **Fail-Closed Write Gate**: Subgrup yang kedaluwarsa otomatis terkunci *read-only* (WebSocket menolak pesan baru & textarea disable).
  ...
  - **UI/UX Aurora Glassmorphism**: `SubGroupListDrawer.tsx` dengan badge hitungan mundur dinamis, badge 🌐 Terbuka / 🔒 Privat, tombol Minta Izin, dan Panel Review Izin bagi Admin; `CreateSubGroupModal.tsx`; serta navigasi kembali `← [Nama Grup Induk]`.
