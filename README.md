...
- [x] **Mobile Viewport Stability Fix (Post-Device Transfer)**: Mengunci `.chat-app-container` dengan `position: fixed` dan `overscroll-behavior: none`, mengganti `scrollIntoView` dengan `containerRef.scrollTo` di ChatWindow, serta reset `window.scrollTo(0,0)` saat transisi room agar tampilan chat tidak naik ke atas setelah penutupan modal transfer.
