...
- [x] **E2EE Single Active Device & Key Conflict Guard (Fase 7 Milestone 7.5)**: Pelacakan `active_device_id` & `key_version` di database, proteksi HTTP 409 Conflict rejection, rotasi kunci resmi `/api/users/public-key/reset`, dan modal dialog UI interaktif (`DeviceConflictModal.tsx`) untuk memastikan Safety Number 100% konsisten antar perangkat.
- [x] **E2EE Device Migration via QR Code (Fase 7 Milestone 7.6 / Opsi 2)**: Fitur pemindahan keypair E2EE antar perangkat secara Zero-Knowledge menggunakan QR code ephemeral dengan TTL 5 menit (`POST /api/users/transfer/create` & `POST /api/users/transfer/consume`), enkripsi AES-256-GCM berdasar 256-bit entropy PBKDF2, retrieval atomik 1x pakai di database, komponen `DeviceTransferModal.tsx` dengan fallback kode manual, serta halaman deep link `/transfer?token=...`.
