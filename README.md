...
- [x] **Core Group Chat Engine & Member Management (Fase 8 Milestone 8.2A)**: Sistem percakapan grup lengkap dengan identitas unik (`grp_<UUIDv4>`), dukungan visibilitas **🔒 Privat (Wuzz Cloud)** vs **🌐 Publik** (dengan handle unik `@group_username` dan pencarian global), wizard 2 langkah pembuatan grup (`CreateGroupModal.tsx`), modal pratinjau & konfirmasi gabung grup publik (`GroupPreviewModal.tsx`) standar Telegram/WhatsApp untuk mencegah *accidental auto-join* (DEC-012), gerbang proteksi otorisasi tautan grup privat (`privateGroupDenied` & Aurora Glassmorphism Shield, DEC-013) yang memblokir akses direct link non-anggota, drawer manajemen grup (`GroupInfoDrawer.tsx`) dengan hierarki peran RBAC (`creator`, `admin`, `member`), promosi/demosi admin, kick anggota, edit info grup, leave group konfirmasi modal, header status bar grup Induk dinamis (`StatusBar.tsx`), linimasa pesan grup dengan warna nickname deterministik unik per pengirim (`MessageBubble.tsx`), model retensi media grup berbasis Shared Media Hub TTL 7 hari di server dan client auto-caching di IndexedDB `wuzzchat_media_db`, semantik tanda terima room (`sent`/`delivered`), bypass E2EE fail-closed untuk room grup (siap di-upgrade ke Signal Sender Keys di masa depan), serta kesiapan fondasi database untuk sub-grup & TTL kedaluwarsa (`parent_id`, `expires_at`).
- [x] **Ephemeral Sub-Groups & Forum Topics Lifecycle Engine (Fase 8 Milestone 8.2B & 8.2C)**: Ruang obrolan bertopik di dalam grup induk (dibrand resmi sebagai **Forum & Topik Diskusi**) dengan identitas unik `sub_<UUIDv4>` dan TTL otomatis. Dilengkapi dengan:
  - **Rebranding Forum & Redesain Header (Milestone 8.2C)**: Mengadopsi standar Telegram Forums & Topics, tombol akses cepat `🏛️ Forum`, breadcrumb interaktif subtitle obrolan (`↖ [Grup Induk] • Forum • X anggota`), dan collapsible action menu (`⋮`) di mobile yang menjaga header obrolan 3x lebih lega dan bebas tumpang tindih.
  - **Strict Parent-Membership Gate & RBAC Enforcement**: Proteksi fail-closed yang memastikan hanya pembuat (`creator`) dan admin (`admin`) yang dapat membuat topik forum baru, sementara seluruh anggota aktif grup induk dapat melihat daftar topik, bergabung, dan berpartisipasi di forum. Tombol pembuatan forum di-hidden otomatis bagi anggota biasa.
  - **Forum Access Control (Terbuka vs Privat)**: Dukungan forum 🌐 **Terbuka** (anggota induk dapat langsung gabung) vs 🔒 **Privat** (memerlukan izin admin atau undangan). Semua topik forum tetap dapat dilihat di daftar (*transparent discovery*).
  - **Antrean Izin Bergabung (`conversation_join_requests`)**: Pengguna mengajukan izin via `POST /api/groups/{id}/join-request`, admin meninjau via `GET /api/groups/{id}/join-requests` dan menyetujui/menolak via `POST /api/groups/{id}/join-requests/{requestId}/action`.
  - **Auto-Purge Data Kedaluwarsa**: Ketika topik forum mencapai masa expired, data di `conversation_join_requests` otomatis dihapus tuntas oleh `ExpireSubGroupsBatch`.
  - **Preset Masa Aktif (TTL)**: Default 1 minggu (`7_days`), dengan opsi terbatas 1 minggu dan 1 bulan (`30_days`).
  - **SubGroupTTLWorker (Background Go Daemon)**: Ticker 15 menit berkala mengeksekusi `ExpireSubGroupsBatch` tanpa memblokir socket.
  - **Fail-Closed Write Gate**: Topik forum yang kedaluwarsa otomatis terkunci *read-only* (WebSocket menolak pesan baru & textarea disable).
  - **Kesiapan AI Summary Masa Depan**: Riwayat pesan tersimpan aman (tidak di-hard delete) untuk diringkas oleh AI summary worker di fase mendatang.
  - **Enforcement Identitas Immutable (DEC-008)**: Seluruh perbandingan dan otorisasi menggunakan UUID/ID immutable tanpa bergantung pada nama/username mutable.
  - **UI/UX Aurora Glassmorphism**: `SubGroupListDrawer.tsx` (panel Forum & Topik Diskusi dengan badge sisa waktu real-time, lencana 🌐 Terbuka / 🔒 Privat, tombol Minta Izin, dan Panel Review Izin bagi Admin); `CreateSubGroupModal.tsx`; serta navigasi kembali `← [Nama Grup Induk]`.
...
- [x] **Shared Media Hub for Group Chats & Forum Topics (Fix Premature Media Expiration)**: Pencegahan media kedaluwarsa dini pada obrolan Grup (`grp_...`) dan Forum Topics (`sub_...`). Konfirmasi unduhan (`POST /api/media/ack`) dari satu anggota tidak lagi menghapus berkas fisik dari Supabase Storage dan tidak menandai `media_status = 'expired'`, sehingga seluruh anggota grup dapat melihat dan mengunduh berkas kapan saja selama masa retensi TTL (default 7 hari). Direct Message 1-on-1 tetap mempertahankan arsitektur *Store-and-Forward* instan ($0 server storage cost).
- [x] **Group & Subgroup Multi-User Mention Engine (`@username`) (Fase 8 Milestone 8.5)**: Pemanggilan terarah anggota percakapan di obrolan grup dan forum/subgrup dengan isolasi keanggotaan ketat (hanya anggota ruang aktif yang dapat di-tag), dukungan multi-mention dalam 1 pesan, penegakan arsitektur kekal murni UUID (`mentions: ["UUID", ...]`), popover autocomplete mengambang Aurora Glassmorphic (navigasi keyboard ↑/↓/Enter/Tab & sentuhan mobile min 44px), rendering tag `@username` di linimasa pesan, highlight khusus self-mention (`.mention-tag-self` dan border pendaran cyan), validasi keanggotaan fail-closed di backend Go WebSocket Hub, dan pengiriman notifikasi Web Push prioritas bertag `chat-mention-[room]`.
- [x] **Realtime Engine Scalability & High-ROI Optimizations (Fase 8 Milestone 8.8)**: Peningkatan efisiensi WebSocket Hub dengan Direct Member Lookup $O(M)$ berbasis in-memory membership cache (mengeliminasi loop linier $O(N)$ dan query SQL berulang pada broadcast pesan), sliding-window typing rate limiter (3 event per 2 detik per koneksi) untuk mencegah flood DoS, Delta Offline History Sync dengan parameter checkpoint `since` (`GetRoomHistorySince`) yang memangkas beban query riwayat hingga 90%, serta Client-Side Outbound Queue (FIFO maks 100 pesan) di `WsClient` yang menahan pesan saat socket disconnected dan melakukan auto-flush seketika saat terhubung kembali.
- [x] **Mobile-Ready Reliability — Request ID + ACK Protocol & Server-Side In-Memory Idempotency (Fase 8 Milestone 8.9)**: Protokol transport andal (*deterministic reliable delivery*) dengan korelasi `request_id` dan paket balasan `type: "ack"`, eliminasi *blind pop* pada `outboundQueue` klien (pesan hanya dihapus setelah konfirmasi ACK server diterima sehingga bebas *phantom message loss* saat koneksi seluler labil), serta proteksi **Server-Side Idempotency Guard (DEC-015)** di Hub Go berbasis in-memory cache berpresisi tinggi `UnixNano()` (TTL 2 menit) yang mencegah pesan duplikat di-broadcast atau di-publish ulang saat klien mobile melakukan reconnect/resend.
- [x] **Message Management Suite — Edit, Forward, Pin Chat, Pin Message & In-Chat Search (Fase 8 Milestone 8.3)**: Fitur manajemen pesan komprehensif standar WhatsApp/Telegram:
  - ✏️ **Edit Pesan (Window 15 Menit)**: Pengeditan pesan terkirim dalam batas 15 menit dengan validasi kepemilikan ketat (*sender only*), event WebSocket `message_edited`, inline input UI, label `(diedit)`, dan pembaharuan cache lokal IndexedDB.
  - ↪️ **Forward Pesan (Max 5 Kontak/Grup)**: Penerusan pesan ke 1 s/d 5 obrolan sekaligus via modal `ForwardMessageModal.tsx`, verifikasi keanggotaan penerima, dan lencana `↪ Diteruskan`.
...
- [x] **Backend Logout Endpoint & Consolidated Encrypted Messages Banner (Fase 8 Milestone 8.10)**:
  - 🚪 **Pelepasan Sesi Perangkat Aktif & Proteksi Device-Aware (`POST /api/auth/logout`)**: Penambahan endpoint logout resmi di backend Go yang mengosongkan kolom `users.active_device_id = ''` di database secara kondisional (*device-aware*), mengeliminasi false conflict (HTTP 409) ketika pengguna berpindah ke perangkat baru setelah logout sukarela. Dilengkapi proteksi pembatalan konflik (*local-only cancellation*): jika perangkat penantang/baru memilih *"Batalkan & Keluar"* di modal konflik, frontend hanya mengeksekusi pembersihan lokal tanpa memanggil server sehingga sesi perangkat aktif utama tetap aman terlindungi.
