...
- [x] **Core Group Chat Engine & Member Management (Fase 8 Milestone 8.2A)**: Sistem percakapan grup lengkap dengan identitas unik (`grp_<UUIDv4>`), dukungan visibilitas **🔒 Privat (Wuzz Cloud)** vs **🌐 Publik** (dengan handle unik `@group_username` dan pencarian global), wizard 2 langkah pembuatan grup (`CreateGroupModal.tsx`) dengan integrasi Recent DM Contacts & pencarian anggota langsung (`/api/users/search`), drawer manajemen grup (`GroupInfoDrawer.tsx`) dengan hierarki peran RBAC (`creator`, `admin`, `member`), promosi/demosi admin, kick anggota, edit info grup, leave group konfirmasi modal, header status bar grup Induk dinamis (`StatusBar.tsx`), linimasa pesan grup dengan warna nickname deterministik unik per pengirim (`MessageBubble.tsx`), model retensi media grup berbasis Shared Media Hub TTL 7 hari di server dan client auto-caching di IndexedDB `wuzzchat_media_db`, semantik tanda terima room (`sent`/`delivered`), bypass E2EE fail-closed untuk room grup (siap di-upgrade ke Signal Sender Keys di masa depan), serta kesiapan fondasi database untuk sub-grup & TTL kedaluwarsa (`parent_id`, `expires_at`).
